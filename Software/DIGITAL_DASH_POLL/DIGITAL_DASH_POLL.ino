...
//Globals
CAN_FRAME RMS_message, BMS_message;
...
void loop() {
  ...
  while(Can0.rx_avail()) {
    Can0.get_rx_buff(RMS_message);
    CAN0_interrupt_handler(&RMS_message);
  }
  
  while(Can1.rx_avail()) {
    Can1.get_rx_buff(BMS_message);
    CAN1_interrupt_handler(&BMS_message);
  }
